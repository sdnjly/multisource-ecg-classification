...
def evaluate_model(ref_onehot_labels, binary_outputs, scalar_outputs, raw_classes):
    ...
    classes, weights = load_weights2(weights_file)
    # classes = raw_classes
    ref_onehot_labels[ref_onehot_labels > 0] = 1

    # Evaluate the model by comparing the labels and outputs.
    print('Evaluating model...')

    print('- AUROC and AUPRC...')
    auroc, auprc, auroc_classes, auprc_classes = compute_auc(ref_onehot_labels, scalar_outputs)

    print('- Accuracy...')
    accuracy = compute_accuracy(ref_onehot_labels, binary_outputs)

    print('- F-measure...')
    f_measure, f_measure_classes, precision, recall = compute_f_measure(ref_onehot_labels, binary_outputs)

    print('- Challenge metric...')
    # arrange to scored labels
    labels_for_scored = arrange_labels(ref_onehot_labels, raw_classes, classes)
    bianry_predictions_for_scored = arrange_labels(binary_outputs, raw_classes, classes)
    challenge_metric = compute_challenge_metric(weights, labels_for_scored, bianry_predictions_for_scored, classes, normal_class)
    print('Done.')
...
# Arrange labels/predictions according to the scored classes
def arrange_labels(labels, original_classes, scored_classes):
    original_classes_set = set(original_classes)
    arranged_labels = np.zeros((labels.shape[0], len(scored_classes)), dtype='float32')
    id_arranged = []
    id_original = []
    for i in range(len(scored_classes)):
        intersec = original_classes_set.intersection(scored_classes[i])
        if len(intersec) > 0:
            id_arranged.append(i)
            id_original.append(original_classes.index(list(intersec)[0]))

    arranged_labels[:, np.array(id_arranged)] = labels[:, np.array(id_original)]
...
def find_challenge_files_out(label_directory, output_directory):
    label_files = list()
    output_files = list()
...
def replace_equivalent_classes(classes, equivalent_classes):
    ...
    classes = sorted(set(classes)) # Remove duplicated classes
    return classes
...
def get_category_field(category_codes, field_name='Dx'):
    # load summary data
    file_name = "dx_mapping.csv"
    ref_data = pd.read_csv(file_name)

    category_names = []
    for code in category_codes:
        try:
            category_names.append(ref_data.loc[ref_data['SNOMEDCTCode']==int(code), field_name].values[0])
        except:
...
def load_labels(raw_labels, classes, equivalent_classes):
...
def load_outputs(raw_binary_outputs, raw_scalar_outputs, raw_classes, classes, equivalent_classes):
    ...
    #        0.12,        0.34,        0.56
    #
    num_recordings = len(raw_binary_outputs)
    num_classes = len(classes)

    raw_classes = replace_equivalent_classes(raw_classes, equivalent_classes)
    # Load the outputs. Perform basic error checking for the output format.
    tmp_labels = [raw_classes for i in range(num_recordings)]

    tmp_binary_outputs = raw_binary_outputs
    tmp_scalar_outputs = raw_scalar_outputs
...
def compute_f_measure(labels, outputs):
    ...
    for k in range(num_classes):
        ...
        if 2 * tp + fp + fn:
            f_measure[k] = float(2 * tp) / float(2 * tp + fp + fn + 1e-10)
            precision[k] = float(tp) / float(tp + fp + 1e-10)
            recall[k] = float(tp) / float(tp + fn + 1e-10)
        else:
    ...
    return macro_f_measure, f_measure, precision, recall
...
def compute_auc(labels, outputs):
    ...
    for k in range(num_classes):
        ...
        for j in range(num_thresholds):
            if tp[j] + fn[j]:
                tpr[j] = float(tp[j]) / float(tp[j] + fn[j] + 1e-10)
            else:
                tpr[j] = float('nan')
            if fp[j] + tn[j]:
                tnr[j] = float(tn[j]) / float(fp[j] + tn[j] + 1e-10)
            else:
                tnr[j] = float('nan')
            if tp[j] + fp[j]:
                ppv[j] = float(tp[j]) / float(tp[j] + fp[j] + 1e-10)
            else:
                ppv[j] = float('nan')
