...
def find_challenge_files(data_directory):
    ...
    for f in os.listdir(data_directory):
        ...
        if not root.startswith('.') and extension=='.hea':
            ...
            sub_headers, sub_records = find_challenge_files(f)
            header_files.extend(sub_headers)
            recording_files.extend(sub_records)
...
def save_outputs(output_file, recording_id, classes, labels, probabilities):
    ...
    class_string = ','.join('|'.join(list(c)) for c in classes)
